...
用户操作流程：
1. 打开 Options 页面
2. 看到一个预定义的 Token 列表（!, ?, TODO, etc.）
3. 修改这些 Token 的样式属性
4. 在代码中写注释时，输入 "//!" 或 "//TODO"
5. 扩展自动识别并应用样式

关键特点：
- 用户不直接告诉扩展「给我高亮这段文字」
- 用户通过「在注释中写特定 Token」来触发样式
- Token 和样式是预先配置好的对应关系
```

**交互入口**：
- 唯一的入口是 Options 页面中的 Token 列表
- 没有右键菜单
- 没有即时添加功能
...
### 冲突 1：触发方式不同 → **解决方案：Token 同时应用 Foreground + Background**

| 场景 | Better Comments Plus | Highlighter |
|------|---------------------|-------------|
| 用户在注释中写 "!" | 自动变红 | 无反应（除非预先配置了规则） |
| 用户选中 "TODO" 右键 | 无菜单项 | 弹出创建规则对话框 |

**决策**：Token 同时应用 Foreground + Background（如果配置了 Background）
...
### 冲突 2：配置层级不同 → **解决方案：Token 支持 Solution 级别，共用数据结构**
...
│   ├─ Token Rules（原 BCP 的 Token）
│   └─ Text Rules（原 Highlighter 的规则）
│
...
│  Basic Info:                                                │
│  ─────────────────────────────────────────────────────────  │
│  Display Name: [Important Comment                     ]     │
│  Match Criteria: [!                                   ]     │
│                                                             │
...
```
1. 写注释 "//! Important"
2. 自动显示红色（如果 Token "!" 已配置）
```

**新方案**：
```
1. 写注释 "//! Important"
2. 自动显示红色 + 黄色背景（如果 Token "!" 配置了两者）
3. 如果 Token "!" 未配置，可以：
   - 选中 "!" → 右键 → Create/Edit Styles Rule → 配置并创建
```
...
**决策**：
- ✅ **保留**预定义 Token 列表（!, ?, TODO, FIXME, NOTE 等）
- ✅ **可以删除**用户不需要的预定义 Token

**预定义列表（Global Rules 默认值）**：

| Token | Display Name | Foreground | Background |
|-------|-------------|------------|------------|
| ! | Important | 红色 + 粗体 | 禁用 |
| ? | Question | 蓝色 | 禁用 |
| TODO | Task | 绿色 | 禁用 |
...
1. **Token 同时应用 Foreground + Background** ✓
2. **Token 支持 Solution 级别，共用数据结构** ✓
3. **右键菜单统一为「Create/Edit Styles Rule」，同时支持两者配置** ✓

### 新方案的优势

1. **统一概念**：不再区分 "Token" 和 "Highlight Rule"，统一为 "Style Rule"
2. **灵活配置**：每个规则可以独立配置 Foreground 和 Background
3. **统一入口**：右键菜单和 Options 页面使用相同的编辑对话框
4. **向后兼容**：预定义 Token 列表保留，老用户习惯不受影响

### 下一步

需要确认：
1. 右键创建规则时的默认启用状态（Foreground/Background 默认开/关？）
2. 是否保留预定义 Token 列表？
3. 规则优先级细节
